...
| `hook/build.dart` | **Native hook:** Rust build + FFmpeg/ffprobe; optional **`localBuild: false`** uses repo `platform-builds/` |
...
## Prebuilt natives (`localBuild`)

Apps can skip compiling Rust in the hook and copy from **`platform-builds/<os>/<triple>/`** at the repository root (next to `media_dart/`).
