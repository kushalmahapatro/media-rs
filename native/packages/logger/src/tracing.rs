...
/// not be changed afterward, i.e. the level and target passed for second and
...
pub fn log_event(
    ...
    static CALLSITES: RwLock<BTreeMap<MetadataId, &'static DefaultCallsite>> =
    ...
    if span_or_event_enabled(callsite) {
        ...
        let message_field = fields.field("message").expect("Expected field 'message'");
        ...
        tracing::warn!("Callsite not enabled");
    }
}
...
fn get_or_init_metadata(
    rwlock: &RwLock<BTreeMap<MetadataId, &'static DefaultCallsite>>,
    id: MetadataId,
    field_names: FieldNames,
    meta_kind: MetadataKind,
) -> &'static DefaultCallsite {
    rwlock.write().unwrap().entry(id).or_insert_with_key(|id| {
        let callsite = Arc::new(LateInitCallSite(OnceCell::new()));
        let callsite_static: &'static LateInitCallSite = Box::leak(Box::new(callsite));
        let metadata = Box::leak(Box::new(tracing::Metadata::new(
            ...
            id.line,
            None, // module path
            FieldSet::new(field_names, identify_callsite!(callsite_static)),
            meta_kind,
        )));
        callsite_static
            .0
...
fn span_or_event_enabled(call_site: &'static DefaultCallsite) -> bool {
    use tracing::{
        dispatcher,
        level_filters::{LevelFilter, STATIC_MAX_LEVEL},
    };

    let meta = call_site.metadata();
    let level = *meta.level();

    if level > STATIC_MAX_LEVEL || level > LevelFilter::current() {
        false
    } else {
        let interest = call_site.interest();
        interest.is_always()
...
//     /// callsite and can not be changed afterward, i.e. the level and
