...
# macOS uses VideoToolbox as the primary H.264 encoder. We do not build OpenH264 here.
if [ "${1:-}" = "--skip-openh264" ]; then
    echo "Skipping OpenH264 build (--skip-openh264 flag set)"
    echo ""
fi

# Download Source (ensure it's not corrupted/incomplete)
cd "$SOURCE_DIR"
if [ ! -d "ffmpeg-8.0.1" ] || [ ! -f "ffmpeg-8.0.1/ffbuild/common.mak" ]; then
    if [ -d "ffmpeg-8.0.1" ]; then
        echo "Existing ffmpeg-8.0.1 directory is incomplete or corrupted. Removing..."
        rm -rf ffmpeg-8.0.1
    fi
...
for ARCH in "${ARCHS[@]}"; do
    ...
    BUILD_DIR="$GENERATED_DIR/ffmpeg_build_$ARCH"
    mkdir -p "$BUILD_DIR"
    ...
    # If OpenH264 was found, ensure pkg-config can locate it
    if [ -n "$OPENH264_ENABLED" ] && [ -d "$OPENH264_DIR/lib/pkgconfig" ]; then
    ...
    if [ "$ARCH" == "arm64" ]; then
        ...
        # Disable x86 assembly to avoid requiring nasm on the host
...
cp -r "$GENERATED_DIR/ffmpeg_build_arm64/include/"* "$INSTALL_DIR/include/"

# Merge Libs
for LIB in "${LIB_NAMES[@]}"; do
    echo "Lipo-ing $LIB.a..."
    lipo -create \
        "$GENERATED_DIR/ffmpeg_build_arm64/lib/$LIB.a" \
        "$GENERATED_DIR/ffmpeg_build_x86_64/lib/$LIB.a" \
...
cp "$GENERATED_DIR/ffmpeg_build_arm64/lib/pkgconfig/"*.pc "$INSTALL_DIR/lib/pkgconfig/"
