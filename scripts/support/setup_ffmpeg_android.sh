...
# Allow either ANDROID_NDK_HOME or ANDROID_NDK_ROOT (common in different setups)
...
build_android() {
    ...
    # Avoid stale *generated* ffbuild artifacts when configure fails.
    # Important: do NOT delete the whole ffbuild/ directory; it also contains make fragments needed for builds.
    rm -f ffbuild/.config ffbuild/config.fate ffbuild/config.log ffbuild/config.mak ffbuild/config.sh 2>/dev/null || true
    ...
    if [ -d "$OPENH264_DIR" ] && [ -f "$OPENH264_DIR/lib/libopenh264.a" ]; then
        ...
        # Add OpenH264 paths to compiler flags (will be used in CONFIGURE_FLAGS)
        EXTRA_CFLAGS="$EXTRA_CFLAGS -I$OPENH264_DIR/include"
        EXTRA_LDFLAGS="$EXTRA_LDFLAGS -L$OPENH264_DIR/lib"
        ...
        if [ -d "$OPENH264_PKG_DIR" ]; then
            ...
            export PKG_CONFIG_PATH="$OPENH264_PKG_DIR"
            # Critical for cross-compilation: allow pkg-config to work across platforms.
            export PKG_CONFIG_ALLOW_CROSS=1
            
            # Verify pkg-config can find it (for debugging)
            if PKG_CONFIG_ALLOW_CROSS=1 pkg-config --exists openh264 2>/dev/null; then
                echo "✓ pkg-config successfully found OpenH264"
                # Test version check that FFmpeg uses
                if PKG_CONFIG_ALLOW_CROSS=1 pkg-config --exists --atleast-version=1.3.0 openh264 2>/dev/null; then
                    echo "✓ pkg-config version check passed (>= 1.3.0)"
                else
                    echo "⚠ Warning: pkg-config version check failed, but library exists"
                    VERSION=$(PKG_CONFIG_ALLOW_CROSS=1 pkg-config --modversion openh264 2>/dev/null || echo "unknown")
        ...
        echo "ERROR: OpenH264 not found for $ABI at: $OPENH264_DIR"
        echo "H.264 software encoding on Android requires OpenH264."
        echo "Run: ./setup_all.sh --android"
        return 1
    fi
    ...
    CONFIGURE_FLAGS=(
        ...
        --enable-jni
        --disable-mediacodec
        # MediaCodec disabled for licensing clarity - use OpenH264 instead (has patent coverage from Cisco)
        # Uncomment the following lines if you want hardware encoding via MediaCodec:
        # --enable-mediacodec
    ...
    # OpenH264 paths are already in EXTRA_CFLAGS and EXTRA_LDFLAGS (added above)
    # PKG_CONFIG environment variables are already set (done above)
    # FFmpeg configure will use these paths even if pkg-config fails
    if [ -n "$OPENH264_ENABLED" ]; then
        CONFIGURE_FLAGS+=(--enable-libopenh264)
        # Make it explicit to ensure the encoder is present in the final build.
        CONFIGURE_FLAGS+=(--enable-encoder=libopenh264)
        CONFIGURE_FLAGS+=(--enable-decoder=libopenh264)
        # FFmpeg's configure performs a link test via pkg-config. OpenH264 is C++ and also needs libm.
        # Without these, configure reports "openh264 not found using pkg-config" even though pkg-config itself works.
        CONFIGURE_FLAGS+=(--extra-libs="-lm -lc++_shared")
        echo "OpenH264 enabled: paths added to compiler flags, pkg-config configured"
    fi
    ...
    if [ "$ARCH" == "aarch64" ]; then
        ...
        # Disable x86 assembly to avoid requiring nasm (not available on macOS by default)
        # C implementations will be used instead, which is fine for Android
    ...
    # Run configure and capture output. For this repo, OpenH264 is REQUIRED for Android H.264 encoding.
    echo "Running configure with REQUIRED OpenH264 support..."
    ./configure "${CONFIGURE_FLAGS[@]}" 2>&1 | tee "$BUILD_DIR/configure.log"
    if [ ! -f "$SOURCE_DIR/ffmpeg-8.0.1/ffbuild/config.mak" ]; then
        echo "❌ Error: FFmpeg configure did not produce ffbuild/config.mak"
        echo "See: $BUILD_DIR/configure.log"
                        return 1
                    fi
    if ! grep -q "^CONFIG_LIBOPENH264=yes" "$SOURCE_DIR/ffmpeg-8.0.1/ffbuild/config.mak"; then
        echo "❌ Error: OpenH264 was not enabled in FFmpeg config.mak"
        echo "This will result in missing 'libopenh264' encoder at runtime."
        echo "See: $BUILD_DIR/configure.log and $SOURCE_DIR/ffmpeg-8.0.1/ffbuild/config.log"
            return 1
    fi
    ...
    for PC_FILE in "$INSTALL_ABI_DIR/lib/pkgconfig/"*.pc; do
        # Ensure the pkg-config files point to the INSTALL dir (not the build dir).
        sed -i '' "s|^prefix=.*|prefix=${INSTALL_ABI_DIR}|g" "$PC_FILE"
        sed -i '' "s|^exec_prefix=.*|exec_prefix=\\\${prefix}|g" "$PC_FILE" 2>/dev/null || true
        sed -i '' "s|^libdir=.*|libdir=\\\${prefix}/lib|g" "$PC_FILE" 2>/dev/null || true
        sed -i '' "s|^includedir=.*|includedir=\\\${prefix}/include|g" "$PC_FILE" 2>/dev/null || true
        # When PKG_CONFIG_SYSROOT_DIR is set during cross-compilation, FFmpeg's .pc generation can
        # mistakenly prefix absolute -L paths with the sysroot (resulting in /sysroot/Users/...).
        # Fix that up for OpenH264 paths so consumers (ffmpeg-sys-next) can link correctly.
        if [ -n "${OPENH264_ENABLED:-}" ] && [ -n "${SYSROOT:-}" ] && [ -n "${OPENH264_DIR:-}" ]; then
            sed -i '' "s|\\-L${SYSROOT}${OPENH264_DIR}/lib|\\-L${OPENH264_DIR}/lib|g" "$PC_FILE" 2>/dev/null || true
        fi
    done
}
...
if [ "$1" != "--skip-openh264" ]; then
    ...
    if [ -f "$PROJECT_ROOT/scripts/support/setup_openh264_android.sh" ]; then
        echo "Running OpenH264 build script..."
        (cd "$PROJECT_ROOT" && "$PROJECT_ROOT/scripts/support/setup_openh264_android.sh")
        echo ""
    else
        echo "Warning: OpenH264 build script not found at scripts/support/setup_openh264_android.sh"
