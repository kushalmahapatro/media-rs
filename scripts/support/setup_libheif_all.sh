...
INSTALL_DIR="$GENERATED_DIR/libheif_install"
...
build_libde265() {
    ...
    # Download libde265 if not present or incomplete
    if [ ! -d "libde265" ] || [ ! -f "libde265/CMakeLists.txt" ]; then
        if [ -d "libde265" ]; then
            echo "libde265 directory exists but is incomplete, removing..."
            rm -rf libde265
        fi
    ...
    # Use a per-target build directory to avoid cross-contamination between macOS/iOS builds.
    local SAFE_PLATFORM
    SAFE_PLATFORM=$(echo "$PLATFORM" | tr -c '[:alnum:]' '_' )
    local BUILD_DIR="build_${SAFE_PLATFORM}_${ARCH}"
    rm -rf "$BUILD_DIR"
    mkdir -p "$BUILD_DIR"
    cd "$BUILD_DIR"
    
    echo "Configuring libde265..."
    # iOS builds must not accidentally link host (Homebrew) libraries (e.g. libunwind.dylib from macOS).
    # Some environments export LDFLAGS/LIBRARY_PATH that break CMake's try-compile step.
    if [[ "$PLATFORM" == iOS-* ]]; then
        env -u LDFLAGS -u LIBRARY_PATH -u DYLD_LIBRARY_PATH -u DYLD_FALLBACK_LIBRARY_PATH -u CPATH -u C_INCLUDE_PATH -u CPLUS_INCLUDE_PATH \
        cmake .. \
    ...
    # Build ONLY the static library to avoid compiling/linking CLI tools like `dec265`
    # (those can fail on cross-arch/universal builds due to host linker/runtime libs).
    ...
    # CMake install for libde265 tries to also install the `dec265` CLI binary even when it's disabled,
    # which can fail. We only need the static library + headers for libheif, so do a minimal manual install.
    mkdir -p "$INSTALL_PREFIX/lib" "$INSTALL_PREFIX/include/libde265" "$INSTALL_PREFIX/lib/pkgconfig"
    FOUND_LIB=$(find . -maxdepth 4 -name "libde265.a" -type f 2>/dev/null | head -1)
    if [ -z "$FOUND_LIB" ]; then
        echo "Error: libde265.a not found after build"
        return 1
    fi
    cp -f "$FOUND_LIB" "$INSTALL_PREFIX/lib/libde265.a"
    # Headers (some are generated into the install tree normally; copy from source/build if available)
    cp -f ../libde265/de265.h ../libde265/en265.h "$INSTALL_PREFIX/include/libde265/" 2>/dev/null || true
    if [ -f "$INSTALL_PREFIX/include/libde265/de265-version.h" ]; then
        true
    else
        FOUND_VER=$(find . -maxdepth 6 -name "de265-version.h" -type f 2>/dev/null | head -1)
        if [ -n "$FOUND_VER" ]; then
            cp -f "$FOUND_VER" "$INSTALL_PREFIX/include/libde265/de265-version.h"
        fi
    fi
    cat > "$INSTALL_PREFIX/lib/pkgconfig/libde265.pc" <<EOF
...
build_macos() {
    ...
    for ARCH in "${ARCHS[@]}"; do
        ...
        BUILD_DIR="$THIRD_PARTY_DIR/generated/libheif_build_macos_$ARCH"
        mkdir -p "$BUILD_DIR"
...
build_ios() {
    ...
    for TARGET in "${TARGETS[@]}"; do
        ...
        # Force Xcode toolchain compilers for iOS targets (avoid picking up host toolchain flags/libs).
        IOS_CC=$(xcrun --sdk $PLATFORM --find clang)
        IOS_CXX=$(xcrun --sdk $PLATFORM --find clang++)
        
        BUILD_DIR="$THIRD_PARTY_DIR/generated/libheif_build_ios_${ARCH}_${TYPE}"
        mkdir -p "$BUILD_DIR"
...
build_android() {
    ...
    for ABI in "${ABIS[@]}"; do
        ...
        BUILD_DIR="$THIRD_PARTY_DIR/generated/libheif_build_android_$ABI"
        mkdir -p "$BUILD_DIR"
        ...
        # Sanitize environment: Homebrew toolchains often export LDFLAGS/LIBRARY_PATH that inject
        # host libraries (e.g. -L/opt/homebrew/opt/llvm/lib/unwind -lunwind) into NDK link steps.
