...
fn find_sysroot() -> Option<String> {
    // NOTE:
    // - Even when we are NOT building FFmpeg from source (i.e. using prebuilt libs via FFMPEG_DIR),
    //   we still run bindgen. For iOS (and Android), bindgen must see the target SDK/sysroot to
    //   resolve standard headers like errno.h.
    // - The original logic gated sysroot discovery behind CARGO_FEATURE_BUILD, which breaks iOS
    //   builds using prebuilt FFmpeg.
    ...
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("ios") {
        // Pick the correct SDK depending on whether we’re building for simulator or device.
        // Rust uses target triples like:
        // - aarch64-apple-ios         (device)
        // - aarch64-apple-ios-sim     (simulator)
        let sdk = if target.contains("apple-ios-sim") || target.contains("ios-sim") {
            "iphonesimulator"
        } else {
            "iphoneos"
        };
    ...
    // For other platforms, only bother finding sysroot when we are actually building FFmpeg from source.
...
fn check_features(
        ...
        ffmpeg_lavc_versions.iter()
    {
        ...
        // Also declare as a cfg flag (not just a feature)
        println!(r#"cargo:rustc-check-cfg=cfg({ffmpeg_version_flag})"#);
        if &stdout[pos..pos + 1] == "1" {
            // Set both as feature (for backward compatibility) and as cfg flag (for new code)
            println!(r#"cargo:rustc-cfg=feature="{ffmpeg_version_flag}""#);
            println!(r#"cargo:rustc-cfg={ffmpeg_version_flag}"#);
...
fn main() {
    ...
    println!(
        "cargo:warning=DEBUG: TARGET_OS={}",
        env::var("CARGO_CFG_TARGET_OS").unwrap_or_default()
    );
    let ffmpeg_major_version: u32 = env!("CARGO_PKG_VERSION_MAJOR").parse().unwrap();

    // Declare ffmpeg_8_0 as a valid cfg condition early, so it's available for all dependent crates
    println!(r#"cargo:rustc-check-cfg=cfg(ffmpeg_8_0)"#);

    let sysroot = find_sysroot();
    // PRIORITY: Check FFMPEG_DIR FIRST - if set, always use pre-built libraries
    // This prevents building FFmpeg from source when pre-built libraries are available
    // Only build from source if FFMPEG_DIR is NOT set AND BUILD feature is enabled
    let ffmpeg_dir_env = env::var("FFMPEG_DIR");
    let build_feature = env::var("CARGO_FEATURE_BUILD");
    println!(
        "cargo:warning=FFMPEG_DIR={:?}, BUILD={:?}",
        ffmpeg_dir_env.is_ok(),
        build_feature.is_ok()
    );

    // CRITICAL: If FFMPEG_DIR is set, we MUST use pre-built libraries and NEVER build from source
    // This prevents embedding incompatible object files in the .rlib
    if ffmpeg_dir_env.is_ok() {
        // CRITICAL: Change the crate fingerprint by emitting a unique metadata key
        // This forces Cargo to treat this as a different build and invalidates
        // any cached .rlib with embedded object files from previous BUILD-enabled builds
        let ffmpeg_dir_str = ffmpeg_dir_env.as_ref().unwrap();
        // Use a hash of FFMPEG_DIR to create a unique fingerprint
        let mut hasher = DefaultHasher::new();
        ffmpeg_dir_str.hash(&mut hasher);
        let ffmpeg_dir_hash = hasher.finish();
        println!("cargo:rustc-env=FFMPEG_PREBUILT_HASH={}", ffmpeg_dir_hash);
        println!("cargo:rerun-if-env-changed=FFMPEG_DIR");
        // Emit a rerun-if-changed directive to force rebuild when FFMPEG_DIR changes
        // This ensures Cargo rebuilds when switching between pre-built and source builds
        println!("cargo:rerun-if-env-changed=FFMPEG_DIR");
        // Write a marker file that changes when FFMPEG_DIR is set
        // This forces Cargo to treat builds with FFMPEG_DIR as different from BUILD builds
        if let Ok(ref ffmpeg_dir_str) = ffmpeg_dir_env {
            use std::collections::hash_map::DefaultHasher;
            use std::hash::{Hash, Hasher};
            let mut hasher = DefaultHasher::new();
            ffmpeg_dir_str.hash(&mut hasher);
            let hash = hasher.finish();
            // Write a marker file to OUT_DIR that changes based on FFMPEG_DIR
            // This ensures Cargo rebuilds when FFMPEG_DIR changes
            let marker_path = output().join("ffmpeg_prebuilt_marker.txt");
            if let Err(e) = std::fs::write(&marker_path, format!("prebuilt-{:x}", hash)) {
                eprintln!("Warning: Failed to write marker file: {}", e);
            }
            // Tell Cargo to rerun if this marker file changes
            println!("cargo:rerun-if-changed={}", marker_path.to_string_lossy());
            // CRITICAL: Also emit a metadata key that changes based on FFMPEG_DIR
            // This helps Cargo distinguish between pre-built and source builds
            println!("cargo:metadata=ffmpeg_prebuilt_dir={}", ffmpeg_dir_str);
        }
        // Also emit a warning to make it clear we're using pre-built libraries
        println!(
            "cargo:warning=Using pre-built FFmpeg from FFMPEG_DIR - will NOT build from source"
        );
        // CRITICAL: Panic if BUILD feature is enabled when FFMPEG_DIR is set
        // This prevents accidental source builds that would embed incompatible object files
        if build_feature.is_ok() {
            panic!("CRITICAL ERROR: Both FFMPEG_DIR and BUILD feature are set. This is not allowed for Android builds as it would embed incompatible object files in the .rlib. Please remove the BUILD feature or unset FFMPEG_DIR.");
        }
    }
    let include_paths: Vec<PathBuf> = if let Ok(ffmpeg_dir_str) = ffmpeg_dir_env {
        // FFMPEG_DIR is set - use pre-built libraries (highest priority)
        // CRITICAL: Change the crate fingerprint by modifying the links name
        // This forces Cargo to treat this as a completely different build
        // and invalidates any cached .rlib with embedded object files
        println!("cargo:rustc-env=FFMPEG_PREBUILT_DIR={}", ffmpeg_dir_str);
        println!("cargo:rerun-if-env-changed=FFMPEG_DIR");
        // Change the links name to force a different fingerprint
        // This ensures Cargo rebuilds the .rlib completely
        println!("cargo:rustc-env=FFMPEG_LINKS_NAME=ffmpeg_prebuilt");

        // CRITICAL: Delete any FFmpeg source/build directories to prevent accidental builds
        let source_dir = source();
        let search_dir = search();
        if source_dir.exists() {
            println!(
                "cargo:warning=Deleting FFmpeg source directory to prevent accidental builds: {}",
                source_dir.display()
            );
            let _ = fs::remove_dir_all(&source_dir);
        }
        if search_dir.exists() && search_dir.join("lib").join("libavutil.a").exists() {
            println!(
                "cargo:warning=Deleting FFmpeg build directory to prevent accidental builds: {}",
                search_dir.display()
            );
            ...
            && env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("aarch64")
        {
            ...
            // For Android, check ABI-specific subdirectories (arm64-v8a, x86_64, etc.)
            let target_arch = env::var("CARGO_CFG_TARGET_ARCH").unwrap_or_default();
            let android_abi = if target_arch == "aarch64" {
                "arm64-v8a"
            ...
            if !android_abi.is_empty() && ffmpeg_dir.join(format!("lib/{}", android_abi)).exists() {
                println!(
                    "cargo:rustc-link-search=native={}",
                    ffmpeg_dir
                        .join(format!("lib/{}", android_abi))
                        .to_string_lossy()
                );
            } else {
                println!(
        ...
        // CRITICAL: After linking, we need to ensure the .rlib doesn't contain
        // incompatible object files. However, we can't modify the .rlib during build.rs
        // as it's created by Cargo after build.rs completes. The object files must be
        // prevented from being embedded in the first place.
        //
        // CRITICAL: When using pre-built FFmpeg libraries, we must NOT link statically
        // in a way that would embed object files. The 'static' feature should only affect
        // how we link, not whether we embed object files. Since we're using pre-built
        // libraries, we should link them as static libraries but NOT embed their object
        // files into the .rlib.
        //
        // The issue is that when linking static libraries, Cargo may embed object files
        // from those libraries into the .rlib. To prevent this, we need to ensure that
        // the static libraries are linked at the final binary stage, not embedded in the .rlib.
        // However, this is controlled by Cargo's linking behavior, not by build.rs.
        //
        // The real solution is to ensure that the .rlib file doesn't contain these object
        // files in the first place. Since we're using pre-built libraries, they shouldn't
        // be embedded. But if they are, we need to clean them after the build.

        let include_dir = ffmpeg_dir.join("include");

        // Force enable ffmpeg_8_0 cfg flag for Android builds with FFmpeg 8.0.1
        // We know we built FFmpeg 8.0.1 for Android, so enable the cfg flag
        if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("android") {
            // Declare ffmpeg_8_0 as a valid cfg condition
            println!(r#"cargo:rustc-check-cfg=cfg(ffmpeg_8_0)"#);
            println!(r#"cargo:rustc-cfg=ffmpeg_8_0"#);
            println!(r#"cargo:rustc-cfg=feature="ffmpeg_8_0""#); // Also set as feature for backward compatibility
            println!(
                "cargo:warning=Force-enabled ffmpeg_8_0 cfg flag for Android FFmpeg 8.0.1 build"
            );
        } else {
            // For other platforms, try to detect version from headers
            let version_header = include_dir.join("libavcodec").join("version.h");
            if let Ok(header_content) = fs::read_to_string(&version_header) {
                // Extract LIBAVCODEC_VERSION_MAJOR and LIBAVCODEC_VERSION_MINOR
                // Format: #define LIBAVCODEC_VERSION_MAJOR  62
                let major = header_content
                    .lines()
                    .find(|l| l.contains("LIBAVCODEC_VERSION_MAJOR"))
                    .and_then(|l| {
                        // Split by whitespace and find the number
                    ...
                    .unwrap_or(0);
                let minor = header_content
                    .lines()
                    .find(|l| l.contains("LIBAVCODEC_VERSION_MINOR"))
                    .and_then(|l| {
                        let parts: Vec<&str> = l.split_whitespace().collect();
                        for part in parts.iter().rev() {
                            if let Ok(n) = part.parse::<u32>() {
                ...
                println!(
                    "cargo:warning=Detected FFmpeg libavcodec version: {}.{}",
                    major, minor
                );

                // FFmpeg 8.0.1 has LIBAVCODEC_VERSION_MAJOR=62, LIBAVCODEC_VERSION_MINOR=8
                if major > 62 || (major == 62 && minor >= 8) {
                    println!(r#"cargo:rustc-check-cfg=cfg(ffmpeg_8_0)"#);
                    println!(r#"cargo:rustc-cfg=ffmpeg_8_0"#);
                    println!(r#"cargo:rustc-cfg=feature="ffmpeg_8_0""#); // Also set as feature for backward compatibility
                    println!("cargo:warning=Enabled ffmpeg_8_0 cfg flag for FFmpeg 8.0.1");
                }
            } else {
                println!("cargo:warning=Could not read version.h at {:?}, assuming FFmpeg 8.0.1 and enabling ffmpeg_8_0", version_header);
                // Default to FFmpeg 8.0.1 if we can't detect (since we know we built 8.0.1)
                println!(r#"cargo:rustc-check-cfg=cfg(ffmpeg_8_0)"#);
                println!(r#"cargo:rustc-cfg=ffmpeg_8_0"#);
                println!(r#"cargo:rustc-cfg=feature="ffmpeg_8_0""#); // Also set as feature for backward compatibility
            }
        ...
        vec![include_dir]
    } else if env::var("CARGO_FEATURE_BUILD").is_ok() && env::var("FFMPEG_DIR").is_err() {
        // BUILD feature is enabled and FFMPEG_DIR is not set - build from source
        // CRITICAL: If FFMPEG_DIR was set earlier but is now unset, this means we're switching
        // between pre-built and source builds. This can cause incompatible object files in .rlib.
        // For Android, we should NEVER build from source - always use pre-built libraries.
        if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("android") {
            panic!("CRITICAL: FFMPEG_DIR is not set for Android build. Android builds MUST use pre-built FFmpeg libraries to avoid architecture mismatches. Please set FFMPEG_DIR environment variable.");
        }
        println!("cargo:warning=BUILD feature enabled - will build FFmpeg from source");
        println!(
            "cargo:rustc-link-search=native={}",
        ...
        if fs::metadata(search().join("lib").join("libavutil.a")).is_err() {
            println!(
                "cargo:warning=FFmpeg libraries not found, fetching and building from source..."
            );
            println!("cargo:warning=CRITICAL: About to call fetch() and build() - this should NOT happen when FFMPEG_DIR is set!");
            fs::create_dir_all(output()).expect("failed to create build directory");
            fetch().unwrap();
            build(sysroot.as_deref()).unwrap();
        } else {
            println!("cargo:warning=FFmpeg libraries found, skipping build");
        }
    ...
    if statik {
        // Check if VideoToolbox is enabled in FFmpeg by checking for the encoder
        // If using pre-built FFmpeg without VideoToolbox, don't link the framework
        let has_videotoolbox = if env::var("CARGO_FEATURE_BUILD").is_ok() {
            // When building FFmpeg, check if build-videotoolbox feature is enabled
            env::var("CARGO_FEATURE_BUILD_VIDEOTOOLBOX").is_ok()
        } else {
            // When using pre-built FFmpeg, check if h264_videotoolbox encoder exists
            // We can't easily check this at build time, so we'll check an env var
            // Set DISABLE_VIDEOTOOLBOX=1 to skip VideoToolbox linking
            env::var("DISABLE_VIDEOTOOLBOX").is_err()
        };
        ...
        if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
            ...
            if has_videotoolbox {
                frameworks.push("VideoToolbox");
            }
            for f in frameworks {
                println!("cargo:rustc-link-lib=framework={f}");
            }
        } else if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("ios") {
            let mut frameworks = vec![
                "AudioToolbox",
                "AVFoundation",
                "CoreFoundation",
...
// Force rebuild Mon Jan 12 20:53:04 +04 2026
