...
impl Context {
    ...
    pub fn set_threading(&mut self, config: threading::Config) {
        unsafe {
            ...
            #[cfg(all(not(feature = "ffmpeg_6_0"), not(ffmpeg_8_0)))]
            {
    ...
    pub fn threading(&self) -> threading::Config {
        unsafe {
            threading::Config {
                ...
                #[cfg(any(feature = "ffmpeg_6_0", ffmpeg_8_0))]
                safe: false,
            }
        }
    }
...
impl Clone for Context {
    ...
    fn clone_from(&mut self, source: &Self) {
        #[cfg(all(not(feature = "ffmpeg_5_0"), not(ffmpeg_8_0)))]
        unsafe {
            // Removed in ffmpeg >= 5.0.
            avcodec_copy_context(self.as_mut_ptr(), source.as_ptr());
        }
        #[cfg(any(feature = "ffmpeg_5_0", ffmpeg_8_0))]
        {
            // avcodec_copy_context was removed, use avcodec_parameters_copy instead
            // This is a simplified fallback - proper implementation would need parameter copying
            let _ = source;
        }
    }
}
