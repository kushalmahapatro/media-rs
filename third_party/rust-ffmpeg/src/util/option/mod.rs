...
impl From<AVOptionType> for Type {
    fn from(value: AVOptionType) -> Self {
        match value {
            ...
            // FFmpeg 8.0.1 removed AV_OPT_TYPE_CHANNEL_LAYOUT, only CHLAYOUT exists
            // Handle both for compatibility with older FFmpeg versions
            #[cfg(all(not(feature = "ffmpeg_7_0"), not(feature = "ffmpeg_5_1")))]
            AV_OPT_TYPE_CHANNEL_LAYOUT => Type::ChannelLayout,
            #[cfg(any(feature = "ffmpeg_5_1", not(feature = "ffmpeg_7_0")))]
...
impl From<Type> for AVOptionType {
    fn from(value: Type) -> AVOptionType {
        match value {
            ...
            // FFmpeg 8.0.1 renamed AV_OPT_TYPE_CHANNEL_LAYOUT to AV_OPT_TYPE_CHLAYOUT
            // Always use CHLAYOUT for FFmpeg 5.1+ (which includes 8.0.1)
