...
fn install_libheif_by_vcpkg() -> Vec<String> {
    ...
    println!("cargo:warning=libheif-sys build.rs: Checking for LIBHEIF_DIR...");
    if let Ok(libheif_dir) = std::env::var("LIBHEIF_DIR") {
        println!("cargo:warning=libheif-sys: Found LIBHEIF_DIR={}", libheif_dir);
        println!("cargo:rerun-if-env-changed=LIBHEIF_DIR");
        let lib_dir = Path::new(&libheif_dir).join("lib");
        let include_dir = Path::new(&libheif_dir).join("include");
        let pkg_config_dir = lib_dir.join("pkgconfig");
        
        println!("cargo:warning=libheif-sys: lib_dir={}, include_dir={}, pkg_config_dir={}", 
                 lib_dir.display(), include_dir.display(), pkg_config_dir.display());
        
        // Verify directories exist
        if !lib_dir.exists() {
            println!("cargo:warning=libheif-sys: WARNING: lib_dir does not exist: {}", lib_dir.display());
        }
        if !include_dir.exists() {
            println!("cargo:warning=libheif-sys: WARNING: include_dir does not exist: {}", include_dir.display());
        }
        if !pkg_config_dir.exists() {
            println!("cargo:warning=libheif-sys: WARNING: pkg_config_dir does not exist: {}", pkg_config_dir.display());
        }
        
        // Add pkg-config path to environment
        if let Ok(existing_path) = std::env::var("PKG_CONFIG_PATH") {
            let new_path = format!("{};{}", pkg_config_dir.display(), existing_path);
            std::env::set_var("PKG_CONFIG_PATH", &new_path);
            println!("cargo:warning=libheif-sys: Set PKG_CONFIG_PATH={}", new_path);
        } else {
            let new_path = pkg_config_dir.to_string_lossy().to_string();
            std::env::set_var("PKG_CONFIG_PATH", &new_path);
            println!("cargo:warning=libheif-sys: Set PKG_CONFIG_PATH={}", new_path);
        }
        
        // Try to use pkg-config via system-deps (similar to non-Windows)
        println!("cargo:warning=libheif-sys: Attempting to use pkg-config via system-deps...");
        let mut config = system_deps::Config::new();
        match config.probe() {
            Ok(deps) => {
                // pkg-config found libheif, return include paths
                println!("cargo:warning=libheif-sys: SUCCESS: Using libheif from LIBHEIF_DIR via pkg-config");
                return deps
                    .all_include_paths()
                    .iter()
                    .filter_map(|p| p.to_str())
            ...
            Err(e) => {
                // pkg-config failed, fall back to manual linking
                println!("cargo:warning=libheif-sys: pkg-config failed: {:?}, using manual linking from LIBHEIF_DIR", e);
            }
        }
        
        // Manual linking fallback
        println!("cargo:warning=libheif-sys: Using manual linking fallback");
        println!("cargo:rustc-link-search=native={}", lib_dir.display());
        println!("cargo:rustc-link-lib=static=heif");
        println!("cargo:rustc-link-lib=static=de265");
        println!("cargo:include={}", include_dir.display());
        return vec![include_dir.to_string_lossy().to_string()];
    } else {
        println!("cargo:warning=libheif-sys: LIBHEIF_DIR not set, falling back to vcpkg");
        // Print all env vars for debugging
        for (key, value) in std::env::vars() {
            if key.contains("LIBHEIF") || key.contains("PKG_CONFIG") {
                println!("cargo:warning=libheif-sys: env {}={}", key, value);
            }
        }
    }
    
    // Fall back to vcpkg if LIBHEIF_DIR is not set
    println!("cargo:warning=libheif-sys: Attempting vcpkg...");
